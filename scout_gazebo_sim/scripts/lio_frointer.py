...
def mapCallback(msg):
    ...
    #         p.x = Clust[i][j][1]*resolution+(origin_x)
    #         p.y = Clust[i][j][0]*resolution+(origin_y)
    #         pp.append(copy(p))   
...
if __name__ == "__main__":
    try:
        ...
        pub = rospy.Publisher('/frointer', Marker, queue_size=10) 
        # rospy.Subscriber("/map", OccupancyGrid, mapCallback, queue_size=1)
        rospy.Subscriber("/robot2/map", OccupancyGrid, mapCallback, queue_size=1)
        rospy.spin()
